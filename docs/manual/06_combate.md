...
**A exceção que confirma a escala.** Existe um tipo de operação de arma que acontece exatamente no alcance 0, e ele não contradiz nada do que foi dito — porque não é tiro. Note que isso não cria uma quinta classe de arma: as classes continuam sendo quatro, e a carga de profundidade é antinavio como qualquer outra. É armamento lançado sobre o setor onde você já está, contra alguém que está **em outro andar** do mesmo hexágono. A carga de profundidade da fragata é o caso puro: o navio não mira o submarino, ele passa por cima e larga a carga na água.
...
Verdes (Vantagem — iniciativa do atacante): 1,0 exato » +1 extra = 2 eliminações, que é exatamente o efetivo restante do alvo — qualquer excedente seria irrelevante.
