...
Rodada de calibracao em partida Humano vs AI, com foco no comportamento da AI vermelha no inicio do T2 e T3. O objetivo foi validar se o pipeline de Intel, Planner, Operations, Shopping e Command Service esta interpretando corretamente a pressao humana, as capturas iniciais e a divisao territorial do mapa.

## Leitura da AI Intel

- `Acoes recentes` representa eventos registrados no log dentro da janela configurada, nao quantidade de unidades nem poder de fogo. No T3, `inimigo=18` indica volume de jogadas recentes do humano.
...
- Bravo recebeu demanda de dois capturadores e um assalto, coerente com pressao no territorio humano.
...
- O stance ofensivo ainda depende demais de inimigos visiveis. Com FoW ocultando o humano, a AI pode manter postura agressiva mesmo quando a Intel historica aponta pressao alta.
...
O pipeline esta coerente de ponta a ponta: Intel gera sinais, Planner cria objetivos, Operations transforma objetivos em necessidades e Shopping reage a lacunas. A calibracao principal agora e de prioridade macro: setores quentes como Alpha precisam furar o cap quando representam captura humana ativa, e o stance deve ponderar Intel historica quando a visibilidade estiver baixa.
