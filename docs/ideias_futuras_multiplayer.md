...
1. **Pacote de turno serializado e versionado**: log de ações do turno + hash do estado final (+ savegame completo como seguro).
...
5. **Caça ao não-determinismo residual**: inimigos clássicos = ordem de iteração (`FindObjectsByType`, `Dictionary`) influenciando DECISÕES durante execução. O replay executa por InstanceId (base sólida); o hash da peça 2 é o detector do que escapar.
...
1. Pacote de turno + hash (engenharia de verdade)
2. Aplicação do pacote como "modo AI" (engenharia de verdade)
3. Resumo de chegada (UI sobre o que existe)
4. Replay pulável do turno inimigo (UI sobre o que existe)
