...
Nenhuma arma de combate direto atinge o próprio hexágono. O tiro começa no alcance 1.
...
Por isso um navio e um submarino coexistem no mesmo hexágono sem trocar tiros. Por isso uma aeronave sobrevoa tropas inimigas sem ser incomodada por elas. Estar junto no mapa não é estar ao alcance.

**A exceção que confirma a escala.** Existe uma classe de arma que opera exatamente no alcance 0, e ela não contradiz nada do que foi dito — porque não é tiro. É armamento lançado sobre o setor onde você já está, contra alguém que está **em outro andar** do mesmo hexágono. A carga de profundidade da fragata é o caso puro: o navio não mira o submarino, ele passa por cima e larga a carga na água.

Repare no que a distância horizontal zero significa aqui. Não é "eles estão colados" — é "a separação entre eles é vertical". A superfície e a profundidade dividem o mesmo setor, e só uma arma feita para atravessar essa fronteira alcança de um andar ao outro.

Duas consequências caem de graça, e as duas são corretas para o que a arma representa:
...
**A distância 1 é lei, não circunstância.** Não existe revide a dois hexágonos em nenhuma situação do jogo, com nenhuma arma. As consequências são grandes e vale enunciá-las uma a uma: um obuseiro atacado de longe nunca revida; dois navios de alcance 2 não trocam fogo quando um ataca a dois hexágonos; um míssil de longo alcance é sempre unilateral. Só o combate adjacente é bilateral — todo o resto é execução.

E há uma segunda exigência, mais estreita do que parece: a arma de revide precisa ter **alcance mínimo 1**. Não basta que o hexágono adjacente caiba no alcance dela. Uma arma de alcance 2 a 4 tem o inimigo colado e mesmo assim não responde — ela não foi feita para brigar de perto. Some a isso munição disponível, camada de disparo permitida e domínio do atacante compatível com a arma, e você tem o quadro completo.

A leitura tática que sai daí é direta: **artilharia adjacente é artilharia indefesa**. Quem tem alcance mínimo alto precisa de escolta, porque não é capaz de reagir a quem encostou.
...
Air/Low tem elevação 3, e aqui vai um fato que costuma passar batido: **Air/Low bloqueia linha de visão**. Um helicóptero em baixa altitude não é só um observador privilegiado — ele é um obstáculo no céu, e projeta sombra sobre quem está atrás dele.

Air/High é a única camada transparente do jogo. Ela não bloqueia a visão de ninguém, para ninguém. Nada se esconde atrás dela, porque ela não projeta sombra sobre nada.
...
Os valores acima são derivados da própria qualidade de posição, não digitados um a um — por isso não existe elemento do jogo com bônus fora dessa escala.
...
**Regra.** O número final de eliminações nunca pode exceder o HP que o esquadrão **atacante** possuía no início desta troca. É um teto só, e ele olha para quem atira, não para quem apanha.

Um esquadrão com apenas 1 HP restante só consegue eliminar, no máximo, 1 inimigo — mesmo que o cálculo indique 5 ou mais. Um esquadrão que começou o combate com 4 HP não elimina mais de 4, com qualquer arma do jogo.

Metáfora das bolas de gude: só restam X bolas no copo — logo, só X carabinas podem disparar.

**Sobre o "teto do alvo".** Você vai ouvir falar de um segundo teto — o de não eliminar mais membros do que o alvo possui. Ele não precisa existir como regra: um esquadrão de 3 homens atingido por uma conta de 7 eliminações simplesmente chega a zero, e não há como ficar mais morto que isso. O excedente não é aparado, é irrelevante. O único teto que muda um resultado é o do atacante.

A distinção importa numa situação concreta: quando o excedente vem do teto do atacante, ele **cancela eliminações que teriam acontecido**; quando vem do tamanho do alvo, não cancela nada, porque o alvo já acabou.
...
Aplique o teto (nunca pode ultrapassar o HP que o atacante tinha no início da troca).
...
A Força de Defesa Efetiva também nunca fica abaixo de 1 na hora da divisão. Por pior que seja a situação, sempre existe um divisor — nenhum confronto produz eliminação infinita.

As duas juntas garantem o mesmo princípio: no The Map Room, um confronto sempre tem um resultado finito, e ninguém é matematicamente invencível nem matematicamente inofensivo.
...
O contrapeso existe e vale conhecer, porque muda como você usa transporte sob fogo: **enquanto o transportador está vivo, quem está dentro não morre.** O dano se propaga proporcionalmente para os passageiros, mas nunca abaixo de 1 ponto de vida. Um comboio castigado entrega tropa machucada; um comboio destruído não entrega nada.
...
Antes dos números, a unidade de medida — porque "rodada" é ambígua numa partida com mais de dois times, e aqui ela tem um sentido só. **Toda duração de exposição é contada em turnos do proprietário da unidade.** Não em turnos globais, não em passagens por todos os times.

Para aeronaves furtivas, a exposição dura **uma rodada**: ela vai do disparo até o início do próximo turno do dono, atravessando os turnos de todos os adversários. Elas permanecem na própria camada e voltam a sumir quando você as recebe de volta. Representa velocidade — trocar de setor e reduzir assinatura antes que forças lentas cerquem a área.

Para o submarino, é mais severo, e dura **duas rodadas**: dois turnos jogáveis completos seus. Ele não perde uma marcação — ele muda de camada, emerge, e fica preso na superfície durante esse tempo.
...
E não é só atacar que o traz para cima. **Revidar** também expõe, e **ser atingido** por armamento apropriado faz o mesmo — pelas mesmas duas rodadas, de propósito, e desde que ele tenha perdido efetivo e sobrevivido. Se disparar custasse menos que ser pego, atacar seria mais seguro do que se esconder, e a arma antissubmarino puniria menos que o próprio torpedo do submarino.
...
O que você pode fazer a partir de um destino provisório depende de quanto você já sabe sobre ele, e são três situações distintas:

**Destino visível agora.** Tudo funciona normalmente. Não há o que esconder de você.

**Destino apenas explorado** — você já esteve ali, tem a fotografia do terreno, mas ninguém seu está olhando naquele momento. Ficam liberados o ataque, o desembarque, a captura e a transferência. Embarque, fusão e suprimento continuam calados, porque dependem de saber quem está lá agora, e isso a fotografia não conta.

**Destino nunca explorado** — o preto de verdade. Sobra apenas o ataque, e ainda assim restrito aos alvos que você já conhecia e cujo corredor de tiro esteja inteiramente revelado. É o mínimo para que avançar não seja suicídio automático, sem transformar o cursor numa lanterna.
...
Isso vale para todo mundo, inclusive para quem não vive na superfície. Uma aeronave nasce **pousada** e um submarino nasce **emerso**, em Naval/Surface, ainda que o lugar dele seja o fundo. O porto não tem profundidade para entregar um submarino já submerso, e a unidade nova precisa caber no andar que trava.

O submarino corrige isso sozinho: **ao terminar o primeiro movimento, ele mergulha** para a camada que prefere, sem que você mande. A única coisa que impede é uma trava de camada ativa — se ele estiver preso na superfície por ter atacado ou apanhado, a preferência espera o relógio acabar. Comprar um submarino, portanto, tem um turno de exposição embutido, e é bom não comprá-lo com o inimigo em cima do porto.
...
A partir daqui o documento muda de natureza. O que veio antes ensina o sistema; o que vem agora registra os valores. Estes apêndices existem para que o manual possa ser consultado como fonte única — e para que a narrativa não precise ser interrompida por tabelas toda vez que um número aparece.
...
## Apêndice — Construções

Não existe uma lista fechada de tipos de construção no sistema. Cada prédio é uma ficha própria, e o que ele "é" resulta das propriedades que ele declara — se aceita pouso, se é porto, se é cidade, se presta serviço. Acrescentar um prédio novo ao jogo não exige código novo, pelo mesmo motivo que acrescentar uma unidade não exige: o mundo já sabe responder às etiquetas que ele carrega.

| Nome oficial | Renda | Visão (terreno) | Notas |
|---|---|---|---|
| HQ | 3.000 | 2 | Posição Única. Captura encerra a partida do dono. Fonte logística infinita. |
| Cidade | 1.000 | 1 | A camada de crescimento da economia. |
| Fábrica Leve | 1.500 | 1 | Base da escada de progressão. |
| Fábrica | 1.500 | 1 | Degrau intermediário. Exige Fábrica Leve. |
| Fábrica Pesada | 2.000 | 1 | Exige Fábrica. |
| Aeroporto | 1.500 | 1 | Isenta consumo de aeronave pousada. Decolagem completa. |
| Aeroporto Avançado | 2.000 | 1 | Idem, mais catálogo de elite. Exige Fábrica. |
| Porto Naval | 1.500 | 1 | Aceita Land/Surface e Naval/Surface, um de cada vez. |
| Estação de Trem | 1.000 | 0 | Logística ferroviária. |
| Hidrobase | 1.000 | 1 | Isenta consumo de qualquer aeronave que pouse. Vende só hidroaviões. |
| Logistica Naval (Docas) | 1.000 | 1 | Supridor naval. |
| Barracks | 500 | 0 | Posição Padrão — não melhora defesa. |
| Terminal Rodoviario | 500 | 0 | Supridor terrestre. |

Duas armadilhas de nome, porque a conversa do dia a dia diverge do jogo: o degrau intermediário é **Fábrica**, não "Fábrica Média"; o porto aparece como **Porto Naval**; e o que todo mundo chama de "Docas" aparece como **Logistica Naval**.

Aeroporto e Aeroporto Avançado são idênticos em tudo que é operacional — pouso, decolagem, isenção de consumo, serviços. Diferem em renda, em pontos de captura (30 contra 40), na política de venda e no catálogo: o avançado é o que destrava a aviação especializada.

Fica pendente, para uma próxima revisão deste apêndice, o detalhamento por prédio de: domínios aceitos, qualidade de posição e bônus de defesa, política de venda, catálogo ofertado, serviços e estoque, pontos de captura, pré-requisito de progressão, regras de pouso e comportamento do Trem de Carga.

## Apêndice — Unidades e Armas

*A ser preenchido.* Este apêndice deve conter a ficha completa de cada unidade — classe, domínio preferido, custo, movimento, autonomia e consumo, defesa, níveis de elite, transporte, serviços prestados, sensores e ocultações — e a ficha de cada arma: classe, potência, alcance mínimo e máximo, munição, trajetória e domínios de alvo.

Duas informações que já cabem registrar, porque são citadas no corpo do manual:

As únicas aeronaves com ocultação no arsenal atual são o **Caça Furtivo** e o **Bombardeiro Furtivo**, ambos ocultos exclusivamente em Air/High.

O **Caminhão de Suprimentos** e o **Trem de Carga** são os portadores da etiqueta de reboque, e a artilharia de campanha é quem a exige.

## Apêndice — Matriz de Detecção

*A ser preenchido.* Deve responder, por camada e por tipo de sensor: quem detecta caça furtivo, quem detecta submarino submerso, quem detecta unidades comuns, qual alcance vale para cada camada de alvo, e quais sensores dispensam linha de visão e sob qual condição.

Três regras já fechadas no corpo do manual e que a matriz deve respeitar:

Não existe detecção genérica de furtivo. As etiquetas de ocultação aérea e submarina são mundos separados, e o equipamento que fura uma não fura a outra.

Dispensar linha de visão é privilégio de aeronave olhando para Air/High, e vale só para detecção — nunca para tiro.

Construções revelam terreno, detectam apenas no próprio hexágono e nunca furam ocultação.

A revelação pertence ao time que detectou.
...
**Isenção de consumo no aeroporto.** A isenção é concedida pela presença sobre o hexágono da instalação, sem verificar se a aeronave está de fato pousada. Uma aeronave sobrevoando o próprio aeroporto deixa de pagar autonomia — o que contraria a razão de design da arremetida automática, que existe justamente para manter a aviação operacional dentro do alcance da antiaérea.

**Caminhão e passageiros.** Este manual afirma que apenas construções atendem os passageiros de um transportador. Isso vale para o suprimento prestado em campo, mas não para o lote do Serviço do Comando, onde o caminhão alcança quem está embarcado. Um dos dois caminhos está errado.

**As duas fontes do "duas rodadas".** A emersão forçada por ataque próprio lê a duração da ficha do submarino; a emersão forçada por dano recebido usa um valor fixo do sistema. Hoje ambos valem 2 e o comportamento é coerente — mudar a ficha desalinharia os dois sem aviso.

**Alcance zero e a inteligência artificial.** O alcance 0 é recurso suportado, não brecha — o sensor de mira o trata explicitamente como combate no mesmo hexágono. A IA, porém, ainda não sabe usá-lo: o simulador de combate dela recusa distância 0 de saída, e o chamador compensa isso enviando distância 1 no lugar. O efeito é que a IA avalia um ataque de alcance 0 como se fosse adjacente — supondo um contra-ataque que nunca vai acontecer — e portanto **subestima** o valor da arma. Uma fragata sob controle da IA não vai caçar submarino com carga de profundidade enquanto isso não for tratado.

**Peso de munição sem valor declarado.** Quando uma ficha de serviço não declara o peso por classe, o sistema aplica 3/2/1 por conta própria. É um valor global escondido atrás de um campo configurável — funciona, mas não é visível para quem edita a ficha.
