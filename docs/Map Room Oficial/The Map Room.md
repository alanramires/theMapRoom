# The Map Room
Bem vindo comandante ao "The Map Room" 
Para entender o jogo primeiro você precisa ter uma visão do que ele é
## A Sala de Guerra
...
É ali que você está, dando ordens, atualizando a posição da frota a medida que ela avança e registrando por onde a esquadrilha está sobrevoando, registrando o consumo logístico e dando ordens indiretas para suas tropas.
Você não está no front de batalha, você está na sala de mapas, no estado maior comandando todas as 3 forças militares.
Esse é o "The Map Room"
## Um jogo determinístico
Por estar em uma sala de mapas, elementos como cobertura, flanqueamento, ataque de oportunidade, clima, tempo, veterania, e todos os elementos de RPG não existem nesse sistema. Tambem não existe aleatoriedade, percentual, grind, rng, dailies. 
### O Tabuleiro
Aqui o tabuleiro representa um mapa, o hexagono é grande o bastante para caber uma cidade inteira, ou uma frota de porta aviões. e você tem uma representação de qual força está ocupando aquele setor no momento.  Então se você vê um caça parado em um hexágono entre turnos, ele não está parado no ar, ele está ocupando aquele setor ou território que pode se extender em vários kilometros.
Uma vez que você conseguiu abstrair o que é o "The Map Room" é hora de mergulhar nos conceitos, e o primeiro deles, antes de falar de unidade, é os "Domínios"
# Domínios
...
**O domínio do Ar** se divide em duas camadas. As _Altas Altitudes_ (Air/High) — ar rarefeito, altas velocidades, onde operam caças e bombardeiros. E as _Baixas Altitudes_ (Air/Low) — arrasto ideal para helicópteros e turboélices.

**O domínio da Terra** (Land) contém apenas a _Superfície_ (Land/Surface), onde operam todas as unidades do exército — planícies, florestas, e as construções humanas como fábricas, cidades e estradas.

**O domínio do Mar** (Naval) agrupa dois subdomínios. O _Nível do Mar_ (Naval/Surface), onde navegam as unidades da marinha e se localizam construções como portos navais. E as _Águas Profundas_ (Submarine/Submerged), onde operam unidades abaixo d'água como o Submarino.
...
**Porto Naval** — construído na praia, aceita Land/Surface e Naval/Surface simultaneamente. Você verá tropas e navios ocupando o mesmo hexágono. É a construção que mantém os dois domínios abertos onde o terreno sozinho só teria um.
...
## Estruturas
Construções modificam o domínio do terreno. Estruturas não — elas trabalham _sobre_ o terreno, melhorando acesso e mobilidade sem alterar a natureza do hexágono.

A regra fundamental das estruturas: elas são sempre avaliadas em par com o terreno onde estão. Uma estrada na planície e uma estrada na montanha são combinações diferentes com resultados diferentes. Quando quiser saber se uma ação é possível, pergunte: _qual estrutura, em qual terreno?_
...
Pontes existem em duas variantes. A **ponte alta** permite passagem simultânea de Naval/Surface por baixo e Land/Surface por cima — navios passam enquanto tanques cruzam. A **ponte baixa** permite apenas passagem de Submarine/Submerged por baixo. A escolha de qual ponte construir no cenário define rotas inteiras de ataque e defesa.
...
Na prática, isso significa que a unidade está _no elemento de maior prioridade_ — e é ele que define defesa, acesso e operação.
...
_"Mire nos olhos, príncipe — ele não pode atacar o que não pode ver."_ — Prince of Persia: Sands of Time
...
O espaço aéreo Air/High não bloqueia a visão de ninguém — qualquer unidade no mapa vê o que está em Air/High. Mas ver não significa alcançar. Essa é a próxima questão.
...
Para disparos parabólicos — artilharia, obuseiros — a linha de tiro direta não é avaliada. A trajetória vai por cima dos obstáculos. Mas se o alvo estiver além da sua visão, você precisa de um observador avançado que veja onde ele está e transmita a posição. Sem olhos no alvo, a artilharia atira às cegas.
...
Todo mundo vê o caça em Air/High — mas nem todo mundo consegue atingi-lo.
...
_The Difference of Position and Quality_ — DPQ — é o que determina como um confronto vai se comportar. Não a posição de cada lado isoladamente, mas a diferença entre elas.
...
Em condições idênticas, o jogo premia quem puxa o gatilho primeiro. O The Map Room incentiva a ofensiva — atacar é melhor que esperar.
...
Construções, terrenos e estruturas são indestrutíveis no The Map Room — então armas antiestrutura como bombas guiadas ou mísseis de cruzeiro simplesmente não existem no sistema. Não há alvo para elas.
...
> **Atacante:** _[Classe de unidade] com [classe de arma] ataca [classe de unidade inimiga] com RPS Ataque [valor]_ **Defensor:** _[Classe de unidade inimiga] se defende de [classe de unidade] com [classe de arma] com RPS Defesa [valor]_
...
_"Eu ouvi plate mail +5?"_ — jogador de RPG frustrado pelo sistema determinista
...
_"Não! Não é ponto de vida!"_

O ícone que aparece na tela é um ♥. A leitura imediata é ponto de vida. Mas o que ele representa é outra coisa — o número de membros vivos dentro do token que você está vendo naquele momento.

Ao ver um Soldado com ♥ 10, você não está vendo um soldado com 10 pontos de vida. Você está vendo 10 soldados reunidos, representados por aquele único token na tela. O mesmo vale para aviões, blindados, navios — qualquer esquadrão do jogo.
...
> _Unidades iguais em condições iguais produzem eliminações idênticas._
...
O sistema de Elite não é um número solto que indica veterano ou regular. Ele é uma condição — e só age quando existe diferença de nível entre os combatentes.

Dois Caças A lutando entre si — ambos Elite 1 — não recebem bônus de elite. Os níveis se cancelam. O resultado é o mesmo que dois Caças B — ambos Elite 0 — lutando entre si. Em ambos os casos: **6 eliminações de um lado, 5 do outro.**

Mas quando Caça A enfrenta Caça B — Elite 1 contra Elite 0, mesma classe, mesma arma, mesmo domínio — o combate distorce completamente:

**Caça A elimina 8 · Caça B elimina 2**

Isso justifica os 10.000 de diferença de custo entre eles. Não é o nome, não é o modelo do míssil — é o nível de elite que separa os dois.

> _Uma observação importante: Elite é específico de classe e arma. O modificador de Dog Fight do Caça A se aplica ao confronto entre jatos — não contra um SAM antiaéreo, não contra Infantaria em terra. Nesses casos, o Elite simplesmente não entra no cálculo, a menos que a unidade tenha uma especialização adicional para aquele confronto específico._

_Um veterano de guerra aérea não é automaticamente um veterano de tudo. Ele é excepcional no que foi treinado para fazer._
