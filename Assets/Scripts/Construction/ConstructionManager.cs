...
public class ConstructionManager : MonoBehaviour
{
    ...
    [Tooltip("Setor estrat�gico ao qual esta constru��o pertence. Use Base0-Base3 para areas de base de cada jogador.")]
    [SerializeField] private ConstructionSector sector = ConstructionSector.None;
    [Tooltip("Overrides manuais de eixo, POR SLOT. O mesmo setor pode ser n� do leque de um slot e rally de outro, ent�o cada entrada for�a o eixo para um slot (ou -1 = todos). Sobrep�e o c�lculo por �ngulo do InvasionAxisMap.")]
    ...
    [Tooltip("Este pr�dio serve como ponto de observa��o avan�ada (Forward Observer) para artilharia.")]
    ...
    // Resolve o eixo for�ado para um slot: a entrada do slot exato tem preced�ncia sobre a de -1
    ...
    // usa pra liberar a produ��o. Para FreeMarket/Disabled o ownerSlot � ignorado.
    ...
    private void RefreshSupplierStockAlerts(bool force)
    {
        ...
        // Com unidade (visivel) em cima, suprime — senao os icones leem como
    ...
    private AIRallyAssemblyState ResolveRallyHudState(out bool isMain)
    {
        ...
        // ap�s um load (a supress�o � persistida). Quando a opera��o falha, o monitor limpa o
