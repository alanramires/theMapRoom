...
public partial class AIController : MonoBehaviour
{
    ...
    [Header("Captura")]
    [Tooltip("Impacto do risco na tomada de decisÃ£o (0 = ignora risco, 2 = risco pesa muito)")]
    [SerializeField, Range(0f, 2f)] private float riskDecisionImpact = 0.5f;
    public float RiskDecisionImpact => riskDecisionImpact;

    [Tooltip("Raio em hexes para criar objetivo defensivo num setor conquistado quando inimigo estÃ¡ prÃ³ximo")]
    [SerializeField, Range(1, 8)] private int defenseEnemyRange = 3;
    public int DefenseEnemyRange => defenseEnemyRange;

    [Tooltip("Multiplicador de recrutamento: rogues dentro de defenseEnemyRange Ã— defenseCallRange hexes sÃ£o convocados para defesa")]
    [SerializeField, Range(1, 8)] private int defenseCallRange = 4;
    public int DefenseCallRange => defenseCallRange;

    [Tooltip("Raio em hexes para detectar inimigos prÃ³ximos ao alvo de captura e acionar reforÃ§o SOS")]
    [SerializeField, Range(1, 8)] private int alliesEnemyRange = 2;
    public int AlliesEnemyRange => alliesEnemyRange;

    [Tooltip("Raio em hexes para recrutar rogues como reforÃ§o SOS em captura com desvantagem")]
    [SerializeField, Range(1, 16)] private int alliesCallRange = 8;
    public int AlliesCallRange => alliesCallRange;

    [Tooltip("Ratio inimigo HP / aliado HP para acionar reforÃ§o SOS numa captura (ex: 2 = inimigo tem o dobro do HP)")]
    ...
    [Header("Transporte")]
    [Tooltip("DistÃ¢ncia mÃ­nima em hexes do HQ para que um setor gere slot de transportador no plano e no shopping")]
    ...
    [Tooltip("MÃ¡ximo de objetivos ofensivos simultÃ¢neos (Pending/Pursuing/Capturing). Limita demand de capturadores em mapas grandes.")]
