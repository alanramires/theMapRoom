...
public partial class AIController
{
    ...
    // Cada um desses mÈtodos constrÛi um PlayerAction especÌfico para uma aÁ„o de IA, 
    // preenchendo os campos relevantes com base nos par‚metros fornecidos. 
    // Isso inclui informaÁıes como a unidade envolvida, a equipe, as posiÁıes de origem e destino, 
    // o tipo de aÁ„o sensorial (se aplic·vel), o caminho de movimento (se disponÌvel) 
    // e rÛtulos de depuraÁ„o para facilitar o rastreamento das aÁıes geradas pela IA durante 
    // o desenvolvimento e testes.
    // Esses mÈtodos ajudam a centralizar a lÛgica de construÁ„o de aÁıes de IA, 
    // garantindo consistÍncia e facilitando a manutenÁ„o do cÛdigo. 
    // Ao usar esses mÈtodos, o restante do cÛdigo de IA pode se concentrar na lÛgica de decis„o 
    // e planejamento, delegando a construÁ„o das aÁıes para esses construtores especializados.
...

    {
...

        {
            ...
            DebugLabel     = $"AI Move {unit.InstanceId} ‚Üí {to}",
...

    {
...

        {
            ...
            DebugLabel      = $"AI Attack {unit.InstanceId} ‚Üí {targetId} @ {targetCell}",
...

    {
...

        {
            ...
            DebugLabel       = $"AI Merge {unit.InstanceId} ‚Üí {target.InstanceId}",
        ...
        Dictionary<Vector3Int, List<Vector3Int>> paths = null)
    {
        ...
        // Percorre o path de tr√°s pra frente e usa o √∫ltimo cell que n√£o √© o transporterCell.
        ...
        return new PlayerAction
        {
            ...
            MovementPath     = movementPath,
            DebugLabel       = $"AI Embark {passenger.InstanceId} ‚Üí {transporter.InstanceId} (slot {slotIndex})",
        };
    }

    // O transportador n√£o se move ao desembarcar: o sensor √© avaliado da posi√ß√£o atual
        ...
        List<PodeDesembarcarOption> disembarkOrders)
    {
        var action = new PlayerAction
        {
            ...
            DebugLabel     = $"AI Disembark ‚Üù {transporter.InstanceId} ({disembarkOrders.Count} passageiro(s))",
        };
        ...
        List<Vector3Int> movementPath)
    {
        var action = new PlayerAction
        {
            ...
            SensorAction   = SensorActionType.Disembark,
            MovementPath   = movementPath,
            DebugLabel     = $"AI Disembark ‚Üù {transporter.InstanceId} ({disembarkOrders.Count} passageiro(s)) via {moveTo}",
        };
        foreach (PodeDesembarcarOption order in disembarkOrders)
        {
            Vector3Int targetCell = order.disembarkCell; targetCell.z = 0;
