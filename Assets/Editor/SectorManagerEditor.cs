...
public class SectorManagerEditor : Editor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        if (GUILayout.Button("Desenhar eixos"))
            DrawInvasionAxes(manager);
    ...
    private static ConstructionSector ReadSector(SerializedProperty sectorProp)
    {
        if (sectorProp == null)
            return default;
        // Pelo VALOR, nunca pelo indice: enumValueIndex nao e o indice de
        // Enum.GetValues (castar ou reindexar devolve o setor vizinho, e as bases,
        // que valem 100+, nunca aparecem). O int gravado e o contrato.
        return ConstructionSectorOrder.ReadSector(sectorProp);
    }
    ...
    private static Color AxisColorForSlot(int slot)
    {
        switch (((slot % 3) + 3) % 3)
        {
            case 0:  return new Color(0.20f, 1f, 0.30f, 1f); // verde
            case 1:  return new Color(0.30f, 1f, 0.80f, 1f); // verde-ciano
            default: return new Color(0.70f, 1f, 0.20f, 1f); // verde-lima
        }
    }
